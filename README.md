...
* concurrentHashMap1.在初始化数组的时候，用了cas机制保证线程安全；2.在put元素判断当前位置上是否有元素的时候，用了cas机制保证线程安全；3.在转换红黑树的时候用synchronized锁住数组中的头结点，降低锁的粒度；4.扩容的时候让其他线程来协助扩容，每个线程分16个数组的长度       
...
2.当一个任务进来，判断当前正在运行的线程数是否小于核心线程数：小于的话，创建线程去执行任务；大于等于的话，就把任务放到阻塞队列去。  
...
### 4.线程的状态：
新建--->可运行---》运行--->阻塞---》等待---》锁池---》死亡       
新建：new出来      
可运行：线程调用start()方法    
运行：可运行的线程被cpu调度；转回可运行可以使用thread.yeild()方法   
阻塞：运行的线程sleep，或者join；sleep结束或join结束进入可运行    
等待：运行线程调用wait方法，释放锁    
锁池：等待线程调用notify方法进入锁池，锁池到可运行  
死亡：线程run方法执行完成，或者异常终止     

### 5.锁    
多线程下会产生数据安全性问题，原子性，可见性，有序性问题    
* synchronized解决原子性和可见性问题。1.6之前是重量级锁，代码块之前会获取监视器monitor，执行完再释放。没有抢到monitor的线程进入阻塞状态，等待线程使用完再去争抢锁，很耗费性能。1.6之后优化了，加入了偏向锁，轻量级锁。偏向锁会记录当前线程的id之类的东西，然后下次还是该线程的话就省了一些步骤，节省资源。适用于单线程的情况。轻量级锁是线程交替访问代码块，采用cas机制，获取失败则自旋。适用于并发很低的场景。并发很高的时候会膨胀成重量级锁，monitor锁，耗资源，适用于高并发的场景。
* lock锁。1.5推出的解决方案，比synchronized轻量，需要上锁lock和释放锁unlock。     
* volatile解决可见性问题和有序性问题，内存直读和指令重排。当cpu对volatile修饰的值修改时，通知其他cpu该值已被修改，其他cpu把高速缓存中的值置为无效，然后该cpu把缓存中的值修改为新值，其他cpu重新获取。
* cas机制：比较并替换，主要有三个参数，1.内存中最新的值，2.内存预期值，3.要修改的新值。内存中最新的值用volatile修饰保证时刻读到的都是最新的。然后用预期值和最新值比较，相等的话把内存最新值改为要修改的新值。不相等则重新获取预期值（自旋）。缺点：1.高并发的话自旋次数浪费cpu性能。2.只能保证单个值的安全性。3.aba问题，不确定内存中的值是否被修改过。    
##### lock和synchronized的区别？    
1.synchronized是基于jvm层面实现的，lock是jdk实现的         
2.synchronized不需要释放锁，lock需要释放锁    
3.synchronized比较消耗资源   


#### 1.系统调度线程的两种方式：轮训和抢占
#### 2.多线程编程的原则：   
CPU密集还是IO密集，IO密集适合多线程     
IO密集型线程池最大线程数配置原则：一般2乘cpu核数    
CPU密集型线程池最大线程数配置原则：一般CPU核数
<br>
